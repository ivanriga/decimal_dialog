...
public class NumberPicker extends LinearLayout implements OnClickListener,
	...
	/**
	 * field - reference to the next number in range
	 */
	private NumberPicker NextNumberPicker = null;

	public NumberPicker getNextNumberPicker() {
		return NextNumberPicker;
	}

	public void setNextNumberPicker(NumberPicker nextNumberPicker) {
		this.NextNumberPicker = nextNumberPicker;
	}
	...
	protected void changeCurrent(int current) {
		...
		if (current > mEnd) {
			...
			if (NextNumberPicker != null) {
				current = mStart;
				NextNumberPicker.changeCurrent(NextNumberPicker.mCurrent + 1);
			} else {
				//самый конец
				current = mStart;
			}
			...
			// сброс со следующего разряда
			
			if (CheckParentIsNotzero(this)) {
				current = mEnd;
				NextNumberPicker.changeCurrent(NextNumberPicker.mCurrent - 1);
	...
	private boolean CheckParentIsNotzero(NumberPicker pnCurrent)
	{
		if ((pnCurrent.NextNumberPicker!=null)&&(pnCurrent.mCurrent==0))
		{
			return CheckParentIsNotzero(pnCurrent.NextNumberPicker);
		}
		else
		{
			return pnCurrent.mCurrent>0;
		}
	}
